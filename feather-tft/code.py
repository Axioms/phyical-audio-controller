...
for encoder_index in range(TOTAL_ENCODERS):
    # create Text Label
    tmp_tabel = bitmap_label.Label(terminalio.FONT, scale=2, x=((BMP_WIDTH + BMP_PADDING) * encoder_index), y=LABEL_HEIGHT, text=padString(str(encoder_index * 10), 3))
...
def SeedValues(data):
    seed_index = data_string.index("Seed Values: ")
    values = data_string[(seed_index + 13):].split(",")
